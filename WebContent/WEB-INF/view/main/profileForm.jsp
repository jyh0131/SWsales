...
<script>
	$(function () {
		...
		console.log(no);
		if(no < 10){
			$("input[name=empNo]").val("EE000"+no);
		}else if(no > 9 && no < 100){
			$("input[name=empNo]").val("EE00"+no);
		}else if(no > 99 && no < 1000){
			$("input[name=empNo]").val("EE0"+no);
		}
		
		var deptName = $("input[name=dept]").val();
		console.log(deptName);
		$("#dept").val(deptName).prop("selected", true);
		
		var bTitle = $("input[name=bTitle]").val();
		console.log(bTitle);
		$("#position").val(bTitle).prop("selected", true);
...
<body>
	<section id="section">
		<div id="login">
			...
			<div id="loginbox">
				<form action="${pageContext.request.contextPath}/main/profile.do" method="post">
					<label class="text">사원번호</label><br>
					<input type="text" name="empNo" class="box" readonly><br>
					<input type="hidden" name="no" value="${emp.empNo }">
					
					<label class="text">이름</label><br>
					<input type="text" name="empName" id="empName" class="box" placeholder="실명을 입력하세요" value="${emp.empName }"><br>
					
					<label class="text">부서명</label><br>
					<input type="hidden" name="dept" value="${emp.dNo.deptName }">
					...
					<input type="hidden" name="bTitle" value="${emp.empTitle }">
					...
					<input type="text" name="empId" id="empId" class="box" style="width: 200px" placeholder=">> 중복확인을 해주세요" value="${emp.empId }">
					...
					<input type="password" name="password" id="pass1" class="box" placeholder="비밀번호" value="${emp.empPass }"><br>
					...
					<input type="email" name="empMail" id="email" class="box" placeholder="이메일 주소" value="${emp.empMail }"><br>
					
					<input type="submit" value="수정" id="btnJoin" style="cursor: pointer">
